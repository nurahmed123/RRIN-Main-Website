...
export const ChatFooter: FC<ChatFooterProps> = ({ chatId, currentUserId }) => {
  ...
  const createMessagehandler = async ({
    ...
    try {
      ...
      form.reset({ content: "" }); // Clear the input after submission
    } catch (error) {
      console.error(error);
      toast.error(
                  ...
                  onKeyDown={async (e) => {
                    if (e.key === "Enter" && !e.shiftKey) {
                      ...
                      await form.handleSubmit(async (data) => {
                        await createMessagehandler(data);
                        // Focus after sending message
                        textareaRef.current?.focus(); // Use textareaRef only when needed, e.g., for focus
                      })();
                    }
                  }}
                  rows={1}
