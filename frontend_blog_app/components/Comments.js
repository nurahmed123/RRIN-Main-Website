...
const Comments = ({ slug }) => {
    ...
    useEffect(() => {
        const fetchComments = async () => {
            ...
            try {
                const response = await axios.get(`/api/comment?slug=${slug}&page=${page}`);
    ...
    const postComment = async (ev) => {
        ...
        try {
            ...
            setEditCommentText("");
            setComment("");
            // Reset page and load comments again
            setPage(1);
            setAllComments([]);
            setHasMore(true);
        } catch (error) {
    ...
    return (
        <>
            ...
            <div className="max-w-5xl mx-auto pt-4 pb-8">
                ...
                {loading && <p className="text-center text-gray-500 dark:text-gray-400">Loading more comments...</p>}
                {!hasMore && <p className="text-center text-gray-500 dark:text-gray-400">No more comments to load</p>}
            </div>
        </>
    );
};
